...
def get_parser():
    ...
    parser.add_argument(
        '--config',
        default='config/st_gcn/nturgbd/train_reDesigned.yaml',
    ...
    def train(self, epoch, save_model=True):
        ...
        if (not arg.accumulating_gradients):
            ...
            for batch_idx, (data, label, name) in enumerate(loader):
                ...
                label = Variable(
                    label.long().to(self.output_device), requires_grad=False)
                # data = Variable(
                #     data.float().to(self.output_device), requires_grad=False)
                # label = Variable(
                #     label.long().to(self.output_device), requires_grad=False)
                ...
                if self.arg.cl_mode is not None:
                    output, cl_loss = self.model(data, label,name, get_cl_loss=True)
                else:
                    output = self.model(data, label,name, get_cl_loss=False)
        ...
        for ln in loader_name:
            ...
            for batch_idx, (data, label, name) in enumerate(self.data_loader[ln]):
                ...
                data = Variable(
                    ...
                    volatile=True)
                label = Variable(
                    label.long().to(self.output_device),
                    requires_grad=False,
                    volatile=True)
                # data = Variable(
                #     data.float().to(self.output_device),
