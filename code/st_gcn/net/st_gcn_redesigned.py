...
# new import created for the FR-Head
from .frHead_lib import ST_RenovateNet
...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        if graph is None:
            ...
            # self.A = torch.from_numpy(self.graph.A).float().to(0)
        ...
        conv_init(self.fcn)
    # ____changed____
    # Adding required functions for the FR-Head 
    '''variables needed 
        - self.base_channel -> create in __init__
        - self.num_frame -> create in __init__ , also in the model call
        - self.num_point -> getting imported to init but need to make it self
        - self.num_person -> getting imported to init but need to make it self
        - self.num_class -> available
        - self.cl_version -> add to config file and get it 
    ...
    '''variables needed 
        - x ->  already in forward
        - label-> already in forward
        - multi_cl_weights -> add to config also create in __init__
        -self.fc-> final layer-> pay attention as both final layers are different
       Where to use this function 
        - forward in model
    '''
    ...
    def forward(self, x, label, name , get_cl_loss = False):
        ...
        for i, m in enumerate(self.backbone):
            # add the clonings and save into feat_low , feat_mid , feat_high , feat_fin 
